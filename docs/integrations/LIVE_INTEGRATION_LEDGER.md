...
Known blockers: owner must run `scripts/rc4/bootstrap-fcm-canary-secret.sh` to preprovision the fixed secret-scoped boundary, then exact-main managed foreground/background delivery proof must pass
Next exact step: apply the bounded owner Secret Manager bootstrap, merge the ledger/proof-trigger reconciliation, and let the one-shot bridge dispatch the exact-main foreground/background managed proof
Ledger updated: YES
```
