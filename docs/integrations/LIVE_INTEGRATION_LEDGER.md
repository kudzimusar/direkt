...
| FCM | `IMPLEMENTED_GATED / SYNTHETIC CANARY PENDING` | RC4 source integration adds provider-neutral HTTP v1 delivery through `communications.push.send.v1`, server-only identity-bound token lifecycle, Android foreground/background receipt handling and Android 13+ notification permission controls. Exact-main run `29909954329` on source `b8d62fc0bc1470766dc6ef55aa807fa5ebdbb5c1` proved synthetic Firebase registration after the BoM/retry correction; managed foreground/background delivery remains pending the fixed secret-container bootstrap. Participant registration is source-controlled disabled. |
...
External provisioning: Firebase project direkt-dev-502701; registered Android debug application com.kudzimusar.direkt.debug; FCM API and least-privilege runtime sender role verified by exact-main run 29909954329; fixed Secret Manager canary container/bindings pending owner bootstrap
Repo/source changes: provider-neutral backend FCM HTTP v1 adapter using managed Google identity; durable communications.push.send.v1 outbox with bounded retry/idempotency and invalid-token handling; server-only token table with classification anti-downgrade control; authenticated identity-bound registration/rotation/deletion routes; Android Messaging SDK/service, exact-source foreground/background receipt handling and Android 13+ permission control; source PR #339; Firebase registration stability correction PR #364
Secret Manager names/versions: dedicated empty container `direkt-fcm-canary-token` must be preprovisioned; deployer receives secret-scoped `roles/secretmanager.secretVersionManager`, runtime identity receives secret-scoped `roles/secretmanager.secretAccessor`; each managed proof adds one temporary numeric version, pins that exact version into the private Cloud Run Job and destroys only that version during cleanup; no runtime secret creation/deletion or IAM mutation is permitted
...
Privacy/security checks: no Firebase Analytics; no raw FCM token in API response/audit/logs/artifacts; device-token rows server-only; push payload restricted to bounded synthetic routing identifiers; routes require authenticated account.sessions.manage permission; permanent verifier rejects runtime secret create/delete, Secret Manager IAM mutation and unpinned `latest` token-secret references
...
Known blockers: owner must preprovision `direkt-fcm-canary-token` with only the documented secret-scoped deployer/runtime bindings, then exact-main managed foreground/background delivery proof must pass
Next exact step: complete PR #370 gates/review, apply the bounded owner Secret Manager bootstrap, merge by pinned head, and let the one-shot bridge dispatch the exact-main foreground/background managed proof
Ledger updated: YES
```
