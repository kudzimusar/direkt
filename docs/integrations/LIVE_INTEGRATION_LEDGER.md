...
| Firebase Crashlytics | `IMPLEMENTED_GATED / MANAGED DEVICE CANARY PENDING` | Android SDK and conditional Google Services/Crashlytics plugin path are source-integrated; main manifest collection is false. Only exact-source `synthetic-canary` builds may initialize the default Firebase app and enable collection with fixed non-sensitive keys. Managed fatal/non-fatal emulator plus Crashlytics API evidence remains required. Participant/production telemetry is disabled. |
| Firebase Test Lab | `PLANNED` | RC5 owns CI/device-matrix automation; RC3 uses the existing isolated GitHub-hosted emulator only. |
...
| Firebase Crashlytics Android | `IMPLEMENTED_GATED / MANAGED DEVICE CANARY PENDING` | Preferred Android crash/non-fatal/ANR path is source-integrated. Collection defaults off; no user IDs, unrestricted logs, contact/evidence/private-coordinate/auth-token metadata, Analytics breadcrumbs, FCM, Maps or Test Lab activation. Exact synthetic fatal/non-fatal managed device/API proof is pending. |

Never send raw evidence, auth tokens, cookies, contact data, exact private coordinates or unnecessary free text to telemetry providers. Crashlytics participant/production collection remains disabled until a separate reviewed privacy/data-use decision and exact release revalidation.
...
5. RC3 Crashlytics Android — **ACTIVE CHECKPOINT — IMPLEMENTED_GATED / MANAGED DEVICE CANARY PENDING**. Crashlytics SDK/default-off collection/conditional synthetic-canary build path, fixed privacy-safe canary events and managed emulator/API verifier are source-integrated. Exact-head regressions and managed fatal/non-fatal proof remain before ACTIVE promotion.
6. RC4 FCM.
