...
requireMarkers(source.workspaceController, [
  "@Get('me')", "providerFromActor: true", "@Patch('me/profile')", "@Put('me/services/:categoryKey')", "@Delete('me/services/:categoryKey')", "@Put('me/location')", "@Put('me/availability/:categoryKey')", "@Post('me/upload-intents')", "@Get('me/upload-intents')", "@Post('me/upload-intents/:uploadIntentId/retry')", "@Put('me/upload-intents/:uploadIntentId/interrupted')", "@Post('me/upload-intents/:uploadIntentId/confirm')",
]);
requireMarkers(source.workspaceDto, ["privateBaseLatitude", "publicPremisesConsent", "serviceAreaWkt", "clientIntentKey", "consentConfirmed", "sha256", "retentionClass"]);
requireMarkers(source.workspaceTypes, ["privateObjectKeyExposed: false", "reviewerIdentityExposed: false", "privateRationaleExposed: false", "providerId: string", "representativeRole: ProviderWorkspaceRole"]);
requireMarkers(source.interactionController, ["@Get('provider-workspace/me/enquiries')", "@Get('provider-workspace/me/enquiries/:enquiryId')", "@Post('provider-workspace/me/enquiries/:enquiryId/transitions')", "providerFromActor: true"]);
requireMarkers(source.handoffController, ["@Get('provider-workspace/me/interactions')", "@Get('provider-workspace/me/enquiries/:enquiryId/handoff')", "providerFromActor: true", "masked, consent-scoped contact hint"]);
requireMarkers(source.handoffTypes, ["providerScope: 'actor_resolved'", "rawContactIncluded: false", "customerContactIncluded: false", "privateEvidenceIncluded: false"]);
requireMarkers(source.interactionDto, ["expectedRevision", "policyVersion", "needs_information"]);
requireMarkers(source.reviewController, ["@Get('provider-workspace/me/reviews')", "@Post('provider-workspace/me/reviews/:reviewId/response')", "@Post('provider-workspace/me/reviews/:reviewId/appeals')", "providerFromActor: true"]);
...
requireMarkers(source.providerApi, [
  '"/api/v1/provider-workspace/me"', '"/api/v1/provider-workspace/me/profile"', '"/api/v1/provider-workspace/me/location"', '"/api/v1/provider-workspace/me/upload-intents"', '"/api/v1/provider-workspace/me/enquiries"', '"/api/v1/provider-workspace/me/interactions"', '"/api/v1/provider-workspace/me/reviews"', '"/api/v1/provider-workspace/me/commercial"', 'authorization: `Bearer ${options.accessToken}`', '"X-Serverless-Authorization"', 'cache: "no-store"',
]);
...
if (/body\.providerId|body\.providerScope|body\.representativeRole/.test(source.actionRoute)) throw new Error("W5 browser actions must not accept provider ownership/scope/role from client input");
if (/objectKey|privateObjectKey/.test(source.providerUi + source.interactionUi)) throw new Error("W5 provider UI must not consume or render private storage object keys");
if (!source.providerUi.includes("uploadAndConfirm") || !source.providerUi.includes("await reload()")) throw new Error("W5 upload flow must confirm durable state and reload authoritative backend state");
if (/fetch\([^\n]+provider-workspace|fetch\([^\n]+api\/v1/.test(source.providerUi + source.interactionUi)) throw new Error("Provider UI must use reviewed same-origin BFF routes rather than direct canonical API calls");

process.stdout.write(`${JSON.stringify({ event: "w5_provider_contract_passed", actorResolvedProviderScope: true, privateEvidenceBounded: true, recoverableUploads: true, enquiryRevisionControl: true, maskedHandoffOnly: true, trackedInteractions: true, serverPolicyVersion: true, w5ClosedBoundaryPreserved: true, directApiFromBrowser: false })}\n`);
