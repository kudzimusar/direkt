...
const evidenceClasses = new Set([
  "contact", "identity", "business", "qualification", "licence", "experience", "location", "premises", "field",
]);
  ...
  try {
    ...
    const result = await withAuthenticatedSession(async (session) => {
      ...
      switch (action) {
        ...
        case "remove-service":
          return api.removeService(
            session.accessToken,
            categoryKey(body.categoryKey),
            text(body.reason, 12, 500),
          );
        case "update-availability": {
          const state = enumValue(body.state, availabilityStates);
          const nextAvailableAt = optionalDate(body.nextAvailableAt);
          if (state === "limited" && !nextAvailableAt) {
            throw new ProviderRequestError("Limited availability requires nextAvailableAt.", 400);
          }
        ...
        case "interrupt-upload":
          return api.markUploadInterrupted(
            session.accessToken,
            uuid(body.uploadIntentId),
            pattern(body.errorCode, /^[A-Z][A-Z0-9_]{2,63}$/, 64, "Invalid upload error code."),
          );
        case "confirm-upload": {
        ...
        case "respond-review":
          return api.respondToReview(session.accessToken, uuid(body.reviewId), {
            body: text(body.body, 10, 1000),
            policyVersion: requirePolicy(policyVersion),
          });
        case "appeal-review":
...
function requirePolicy(policy: string | null): string {
  if (!policy) throw new ProviderRequestError("Provider interaction policy is not configured.", 503);
  return policy;
}
...
function profilePatch(body: Record<string, unknown>) {
  ...
  const displayName = optionalText(body.displayName, 2, 160);
  if (displayName) patch.displayName = displayName;
  if (body.operatingModel !== undefined && body.operatingModel !== "") {
    patch.operatingModel = enumValue(body.operatingModel, operatingModels) as "fixed_premises" | "mobile" | "hybrid";
  }
  const localitySummary = optionalText(body.localitySummary, 2, 160);
  if (localitySummary) patch.localitySummary = localitySummary;
  const serviceAreaSummary = optionalText(body.serviceAreaSummary, 2, 240);
  if (serviceAreaSummary) patch.serviceAreaSummary = serviceAreaSummary;
  const registeredBusinessName = optionalText(body.registeredBusinessName, 2, 200);
  if (registeredBusinessName) patch.registeredBusinessName = registeredBusinessName;
  const qualificationSummary = optionalText(body.qualificationSummary, 8, 500);
  if (qualificationSummary) patch.qualificationSummary = qualificationSummary;
  const experienceSummary = optionalText(body.experienceSummary, 8, 500);
  if (experienceSummary) patch.experienceSummary = experienceSummary;
...
function optionalCoordinatePair(latitude: unknown, longitude: unknown, prefix: "privateBase" | "publicPremises") {
  ...
  if (typeof latitude !== "number" || typeof longitude !== "number") {
    throw new ProviderRequestError("Latitude and longitude must be provided together as numbers.", 400);
  }
  if (latitude < -90 || latitude > 90 || longitude < -180 || longitude > 180) {
    throw new ProviderRequestError("Coordinates are outside the valid WGS84 range.", 400);
  }
  return prefix === "privateBase"
    ? { privateBaseLatitude: latitude, privateBaseLongitude: longitude }
    : { publicPremisesLatitude: latitude, publicPremisesLongitude: longitude };
}

function uuid(value: unknown): string {
  if (typeof value !== "string" || !UUID.test(value)) throw new ProviderRequestError("A valid UUID is required.", 400);
  return value;
}

function categoryKey(value: unknown): string {
  if (typeof value !== "string" || !CATEGORY.test(value)) throw new ProviderRequestError("A valid category key is required.", 400);
  return value;
}

function text(value: unknown, min: number, max: number): string {
  if (typeof value !== "string") throw new ProviderRequestError("Text value is required.", 400);
  const normalized = value.trim();
  if (normalized.length < min || normalized.length > max) {
    throw new ProviderRequestError(`Text length must be between ${min} and ${max}.`, 400);
  }
  return normalized;
}

function optionalText(value: unknown, min: number, max: number): string | undefined {
  if (value === undefined || value === null || value === "") return undefined;
  return text(value, min, max);
}

function pattern(value: unknown, regex: RegExp, max: number, message: string): string {
  if (typeof value !== "string") throw new ProviderRequestError(message, 400);
  const normalized = value.trim();
  if (normalized.length > max || !regex.test(normalized)) throw new ProviderRequestError(message, 400);
  return normalized;
}

function integer(value: unknown, min: number, max: number): number {
  if (!Number.isInteger(value) || (value as number) < min || (value as number) > max) {
    throw new ProviderRequestError("A valid integer is required.", 400);
  }
  return value as number;
}

function booleanValue(value: unknown): boolean {
  if (typeof value !== "boolean") throw new ProviderRequestError("A boolean value is required.", 400);
  return value;
}

function enumValue(value: unknown, allowed: Set<string>): string {
  if (typeof value !== "string" || !allowed.has(value)) throw new ProviderRequestError("Unsupported enum value.", 400);
  return value;
}
...
function optionalDate(value: unknown): string | undefined {
  ...
  if (typeof value !== "string" || Number.isNaN(Date.parse(value))) {
    throw new ProviderRequestError("A valid date/time is required.", 400);
  }
