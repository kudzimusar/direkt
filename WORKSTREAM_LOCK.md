...
| Implementation branch | `integration/rc2-sentry-runtime` from RC1 closure merge `772ce87e2b81820a4ecb84d0df430de4b8f830ae`; do not build new integration work on the diverged historical `build/android-v1` branch |
| Stable baseline | RC1 closure merged to `main` at `772ce87e2b81820a4ecb84d0df430de4b8f830ae`; Resend is active only for the proven synthetic managed canary boundary; continuous/participant/production email remains disabled |
| Current task | RC2 — audit existing Sentry provisioning, then integrate separate API/portal telemetry with strict minimization/scrubbing, release/source-map controls, fail-closed kill switches, Secret Manager/runtime binding and managed synthetic canaries while Cloud Logging remains authoritative infrastructure telemetry |
...
- RC2 — Sentry for approved NestJS/Next.js surfaces with strict PII scrubbing, release/source-map controls and kill switch; Cloud Logging remains authoritative infrastructure telemetry. **ACTIVE CHECKPOINT.**
