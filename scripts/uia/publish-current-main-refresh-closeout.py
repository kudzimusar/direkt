...
for value, name in ((SOURCE, "source"),):
    if len(value) != 40 or any(c not in "0123456789abcdef" for c in value):
        raise SystemExit(f"Invalid {name}: {value}")
for value, name in (
    (WEB_RUN, "web run"),
    (ANDROID_RUN, "Android run"),
...
replace(LOCK, "| Implementation branch | `chore/uia-current-main-refresh`, based on the UIA claim merge from exact `main@348aedfdc29d4cc82bcc4296648db844d7fd5e44`. |", f"| Implementation branch | None. UIA refresh PR #{PR_NUMBER} promoted exact source `{SOURCE}` through managed owner-review runtimes. |")
...
|---|---:|---|---|
| Customer/provider browser and BFF | `{WEB_RUN}` | passed | public synthetic-only owner review; canonical private-API/BFF boundary preserved |
...
1. **Visual/synthetic review — complete.** Production-built or equivalent managed surfaces represent the current product without claiming participant data.
...
UIA is closed. Current owner-review surfaces are available through their approved access paths. This receipt proves presentation/access freshness only and does not weaken the Phase 11 `ENTRY_BLOCKED_EXTERNAL` decision.
""", encoding="utf-8")
  ...
  verify:
    ...
    steps:
      ...
      - uses: actions/setup-python@v6
        with:
          python-version: "3.12"
      - name: Verify UIA current-main owner review
