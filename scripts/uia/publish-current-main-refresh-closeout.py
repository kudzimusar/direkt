...
VERIFIER = ROOT / "scripts/uia/verify-current-main-owner-review.py"
CONTRACT = ROOT / ".github/workflows/uia-current-main-owner-review-contract.yml"

SOURCE = os.environ["SOURCE_SHA"]
WEB_RUN = os.environ["WEB_RUN_ID"]
ANDROID_RUN = os.environ["ANDROID_RUN_ID"]
OPERATIONS_RUN = os.environ["OPERATIONS_RUN_ID"]
CANONICAL_RUN = os.environ["CANONICAL_RUN_ID"]
PR_NUMBER = os.environ["PR_NUMBER"]

if len(SOURCE) != 40 or any(c not in "0123456789abcdef" for c in SOURCE):
    raise SystemExit(f"Invalid source: {SOURCE}")
for value, label in ((WEB_RUN, "web"), (ANDROID_RUN, "android"), (OPERATIONS_RUN, "operations"), (CANONICAL_RUN, "canonical"), (PR_NUMBER, "PR")):
...
def replace(path: Path, old: str, new: str) -> None:
    ...
    if old not in text:
        raise SystemExit(f"UIA closeout missing expected text in {path.relative_to(ROOT)}: {old}")
...
replace(LOCK, "| Authorized scope | Refresh the exact-current-main synthetic owner-review browser/PWA, internal Android and IAM-private operations staging surfaces; verify canonical access; publish evidence; close Issue #354. No participant processing, production auth, private evidence activation, external communications, real money or Phase 12 release. |", "| Authorized scope | No active repository write scope. A new explicit claim is required for actual Phase 11 external evidence or participant-backed execution. |")
replace(LOCK, "| Implementation branch | `chore/uia-current-main-refresh`, based on the UIA claim merge from exact `main@348aedfdc29d4cc82bcc4296648db844d7fd5e44`. |", f"| Implementation branch | None. UIA closeout PR #{PR_NUMBER} promoted exact source `{SOURCE}` through managed owner-review runtimes. |")
...
replace(LOCK, "| Governing issue | Issue #354 governs UIA refresh. Issue #112 remains separately open for external entry gates and real pilot evidence. |", "| Governing issue | Issue #112 remains open for external entry gates and real pilot evidence. Issue #354 is closed and preserved. |")
...
replace(PROJECT, "**Active repository write lane:** none; Wave 0 finishing-line controls are closed and actual external-evidence reconciliation is next but unclaimed", "**Active repository write lane:** none; UIA current-main owner review is closed and actual Phase 11 external-evidence reconciliation is next but unclaimed")
...
UIA is closed. This receipt proves current presentation/access only and does not weaken the Phase 11 `ENTRY_BLOCKED_EXTERNAL` decision.
""", encoding="utf-8")
...
for needle in ("UIA owner-review promotion contract — CLOSED AND PRESERVED", "CURRENT-MAIN SYNTHETIC OWNER REVIEW PROVEN", "The repository write lane is RELEASED", "PILOT_ENTRY_APPROVED` remains false"):
    require(LOCK, needle)
require(PROJECT, "UIA current-main owner-review access — **CLOSED AND PRESERVED")
for needle in ("Managed source:** `{SOURCE}`", "`{WEB_RUN}`", "`{ANDROID_RUN}`", "`{OPERATIONS_RUN}`", "`{CANONICAL_RUN}`", "Real Phase 11 participant UAT — not run", "Production release — not authorized", "https://app.direkt.forum", "direkt-internal-testers", "direkt-operations-portal-staging"):
    require(RECEIPT, needle)
for script in ("scripts/rc11/verify-final-integration-closure.py", "scripts/phase11/verify-primary-pilot-readiness.py", "scripts/phase11/evaluate-wave0-gates.py", "scripts/phase11/verify-wave0-finishing-line.py"):
...
CONTRACT.write_text('''name: DIREKT UIA current-main owner-review contract
...
''', encoding="utf-8")
print("UIA_CURRENT_MAIN_REFRESH_CLOSEOUT_PATCH|PASS")
