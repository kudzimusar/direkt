#!/usr/bin/env python3
"""Apply the exact RC6 closure and RC5-resume reconciliation."""
...
def replace_once(path: str, old: str, new: str) -> None:
    ...
    if count != 1:
        raise SystemExit(
            f"{path}: expected exactly one occurrence, found {count}: {old[:120]}"
        )
...
def main() -> int:
    ...
    verifier = "scripts/rc6/verify-whatsapp-contract.py"
    replace_once(
        verifier,
        '''    for needle in (
...
''',
    ...
    closed_lock_needles = (
        ...
        "RC6 implementation contract — CLOSED AND PRESERVED",
        "RC6 exact-current-main managed run `30137700769`",
...
''',
    ...
    replace_once(
        ...
        "**Authoritative as-of date:** 2026-07-25 (Asia/Tokyo)  ",
    )
    replace_once(
        status,
...
```
"""
    replace_once(ledger, anchor, block)
    replace_once(
        ledger,
    ...
    for path in (lock, verifier, status, ledger, project):
        if "\r" in Path(path).read_text(encoding="utf-8"):
            raise SystemExit(f"{path}: CR character introduced")
