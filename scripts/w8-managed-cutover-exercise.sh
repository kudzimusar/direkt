...
curl -fsS -D "${RUNNER_TEMP}/w8-categories.headers" "${WEB_URL}/api/discovery/categories" > "${categories_json}"
...
curl -fsS -D "${bootstrap_headers}" -c "${jar}" "${WEB_URL}/api/auth/bootstrap" > "${bootstrap_json}"
